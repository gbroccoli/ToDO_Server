...
class Config:
	...
	def addStatic(self):
		static_data = conf.get("static", [])  # Получаем список статических данных из conf, если он существует
		...
		""" for item in static_data:
			...
			if all((subpath, directory, name)):
				static_obj = StaticType(subpath=subpath, dir=directory, name=name)
